...
export default function Portfolio({ positions, balance, initialBalance, onClose, onUpdateThesis, onUpdatePolymarketUrl, onUpdateExitNotes, isProcessing = false, sellingPositionId = null, pnlHistory = [] }: PortfolioProps) {
  ...
  // For closed positions, use exitPrice to calculate realized gains
  const realizedPnL = closedPositions.reduce((sum, pos) => {
    if (pos.exitPrice) {
      // Calculate realized P&L: (exitPrice - entryPrice) * shares
      const realizedGain = (pos.exitPrice - pos.entryPrice) * pos.shares;
